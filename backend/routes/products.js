...
async function uploadToCloudinary(buffer) {
  return new Promise((resolve, reject) => {
    cloudinary.uploader.upload_stream(
      { folder: 'shopflow', resource_type: 'image' },
...
router.post('/', auth, upload.single('image'), async (req, res) => {
  try {
    ...
    const product = new Product({ name, category, description, price: parseFloat(price), unit: unit || 'per piece', stock: parseInt(stock) || 0, image: imageUrl });
    await product.save();
